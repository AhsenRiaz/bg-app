...
function Post(props) {
  ...
  async function getLikedUsers() {
    try {
      ...
      const response = await axios.post(
        `http://localhost:3000/post/${post._id}/liked-users`
      );
  ...
  async function likePostAPI() {
    try {
      ...
      let postID = post._id;
      const response = await axios.post(
        `http://localhost:3000/like-posts/${userEmail}`,
        {
          postID: postID,
        }
      );
  ...
  async function unlikePostAPI() {
    try {
      ...
      let postID = post._id;
      const response = await axios.post(
        `http://localhost:3000/unlike-posts/${userEmail}`,
        {
          postID: postID,
        }
      );
  ...
  function renderPosts() {
    return (
      ...
      >
        <TouchableWithoutFeedback
          onPress={() => console.log("Liked Are", likedUsers)}
        >
          <Text
        ...
        <View style={{ flex: 1, justifyContent: "flex-end" }}>
          ...
          {likedUsers ? (
            ...
            >
              <Text style={styles.likeCount}>{likedUsers.length}</Text>
            </TouchableOpacity>
          ) : (
            <></>
          )}
